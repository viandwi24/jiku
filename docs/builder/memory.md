...
## mtcute v0.27 `sendText` `replyTo` must be a plain NUMBER (or Message), never a `{messageId, threadId}` object

`client.sendText(peer, text, { replyTo: <value> })` in mtcute v0.27 overloads `replyTo` as `number | Message` only. A plain object like `{ messageId, threadId }` crashes at `send-common.js:27` with `TypeError: undefined is not an object (evaluating 'params.replyTo?.chat.inputPeer')` — mtcute reads `.chat.inputPeer` off the value when it's not a number, expecting a Message instance. High-level API also does NOT expose TL's `top_msg_id`, so there's no way to simultaneously reply-to-message AND route-to-topic via opts. Use a SINGLE number: prefer the user's message id (Telegram server auto-inherits topic context from the replied message + renders the quote bubble), fall back to threadId only when no message id (routes to topic without quote — mtcute's own `answerText` does this). Same rule applies to outbound sendMessage opts when merging caller-supplied `content.params.replyTo` — normalise any `{messageId}` object to a number before it reaches mtcute.

## Telegram userbot streaming sends must pass `replyTo: <userMsgId>` — mtcute defaults to General + no quote

`handleResolvedEvent` in `user-adapter.ts` hits `client.sendText(peer, '⌛', ...)` at FOUR sites (initial placeholder, overflow continuation placeholder, edit-failure fallback, no-edit final). Each one needs the same `sendOpts` — without `replyTo` the message silently lands in the forum's General topic even when the event came from a topic-scoped binding, AND there's no "replying to…" quote linking the response to the user's message. Hoist `replyToMsgId = ctx.event.ref_keys['message_id']` + `threadId = ctx.event.ref_keys['thread_id']` at top, build `sendOpts.replyTo` as a single number (prefer msgId, fall back to threadId), pass to all four sends. `editMessage` doesn't need the opts — the placeholder already carries the context. Same pattern in non-streaming `sendMessage()` via `target.reply_to_ref_keys.message_id`. Parity reference: `bot-adapter.ts:1679-1692`.

## Telegram userbot mention detection: `isMention` flag + entity scan, with `text_mention | mentionName` alias

Userbot `normalizeInbound` must populate `metadata.bot_mentioned` + `metadata.bot_replied_to` to play nice with `event-router.ts:243-253` `trigger_mode='mention'` binding gate. Layered: `m.isMention === true` is mtcute's authoritative precomputed flag, but cross-check `m.entities[]` as fallback because the flag isn't always consistent across edits/forwards. For user-id text mentions, accept BOTH `type === 'text_mention'` AND `type === 'mentionName'` — mtcute build variance surfaces them under either name (grammy uses `text_mention` at Bot API layer). `bot_replied_to` = `m.replyToMessage.sender.id === myUserId` (falls back to `senderId` field when sender object absent). Metadata fields spread conditionally — only present when true, matching bot adapter shape so `!== true` checks pass identically for both adapters. Without this, every userbot binding with `trigger_mode='mention'` + empty tokens silently drops all group events.

## Telegram userbot topic extraction: `isTopicMessage + replyToMessage.threadId`, not just `replyToMessage`

mtcute's inbound `Message` encodes forum-topic membership via TWO fields: `isTopicMessage: boolean` (authoritative flag) and `replyToMessage.threadId: number` (the topic's root message id). Both must be checked — `replyToMessage.threadId` also appears on normal replies that happen to be inside a topic, so on its own it doesn't tell you the message is a topic-root member. Canonical extraction: `const threadId = m.isTopicMessage && m.replyToMessage?.threadId != null ? String(m.replyToMessage.threadId) : undefined`. This becomes `ref_keys.thread_id` + feeds `scope_key = group:{chat}:topic:{thread}` (parity with bot adapter grammar). Outbound to mtcute takes `opts.replyTo = { threadId }` — accepts a number (reply-to-message-id), an object (full `InputReplyTo` with `messageId`/`threadId`/`chatId`), or undefined. General topic messages are NOT flagged `isTopicMessage` and have no threadId — same semantics as grammy's `message_thread_id` skipping General.

## Tool handlers can call the agent's LLM via `ctx.runtime.llm.generate(...)`

Plan 26 added `RuntimeContext.llm?: LLMBridge` (`packages/types/src/index.ts`). The runner (`packages/core/src/runner.ts`) binds it to the agent's resolved provider/model via AI SDK `generateText` — so any plugin that wants to call an LLM inside `execute()` just does `const text = await toolCtx.runtime.llm?.generate(prompt, { system?, provider?, model?, temperature?, maxTokens? })`. No need to declare a `depends: [LlmProvider]` or inject a client — the bridge is the canonical access point (ADR-104). Pattern used by `jiku.code-runtime` for prompt-mode codegen. `llm` is optional on the type so guard with `if (!ctx.runtime.llm) throw ...` in flows that genuinely need it. Don't forget the opts argument if you want to route codegen to a cheaper model than whatever the agent is running on.
